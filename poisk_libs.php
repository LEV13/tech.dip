...
//-------------------------------------
	$type_sql_add = mysql_query(
				"SELECT
					`lib_types`.`id` AS `types_id`,
					`lib_types`.`name` AS `types_name`
					FROM `lib_types`
					WHERE `lib_types`.`name` = '$type_name'
				");
...
if ($type_name != NULL) {$type_name_sql = "`libs`.`lib_type_id` = '$type_add_id'";} else {$type_name_sql = NULL;} // Тип элемента
//if ($tags_name != NULL) {$tags_name_sql = "`elements`.`id` = '$tags_elem_add_id'";} else {$tags_name_sql = NULL;}
if ($text_name != NULL) {$text_name_sql = "`libs`.`name` like '%$text_name%'";} else {$text_name_sql = NULL;} // Текст элемента
if ($code_name != NULL) {$code_name_sql = "`libs`.`code` = '$code_name'";} else {$code_name_sql = NULL;} // Код элемента
...
if ($type_name == NULL and $text_name == NULL and $code_name == NULL) {$dop_add = "`libs`.`lib_type_id` = '$type_add_id'";} else {$dop_add = NULL;}
if ($type_name != NULL or $text_name != NULL or $code_name != NULL) {$and_0 = " AND "; } else {$and_0 = NULL;}

$query = mysql_query(
	"SELECT
	`libs`.`id` AS `libs_id`,
	`libs`.`name` AS `libs_name`,
	`libs`.`simulator_id` AS `libs_simulator_id`,
	`libs`.`description` AS `element_description`,
	`libs`.`code` AS `libs_code`,
	`libs`.`lib_type_id` AS `libs_lib_type_id`,
...
$query_s = mysql_query(
	...
	echo "
	<a href='/?c=".$row_s['lib_simulator_id_s']."&s=".$row_s['lib_simulator_id_s']."&l=".$row_s['element_lib_id_s']."'>
	<div class='cerch_elem_up'>
	<div class='cerch_elem1'><img height='25px' src='".$row['element_icon']."'/></div>
	<div class='cerch_elem2'>".$row['libs_name']."</div>
	<div class='cerch_elem3'>".$row['libs_code']."</div>
	<div class='cerch_elem4'>".$row['lib_types_name']."</div>
	<div class='cerch_elem5'>".$row['libs_description']."</div>
	</a>
	</div>
	";
}
